...
# PROTEÇÃO CSRF
# =========================================================
# Cada sessão recebe um token aleatório. Toda requisição POST precisa
# devolver esse mesmo token em request.form["csrf_token"] (incluindo
# chamadas via fetch/FormData, como o autosave), senão é rejeitada.
...
# Garante que as migrações do banco sejam executadas também no PythonAnywhere/WSGI.
# O servidor web importa este arquivo e não passa pelo bloco __main__.
criar_banco()
...
def dashboard():
    ...
    # Painel 1: carenagens efetivamente conferidas no período.
    # Não depende de a entrega já ter sido finalizada: qualquer quantidade
    # conferida registrada pelo operador entra no total.
    resumo = c.execute("""
        SELECT
    ...
    # Painel 2: somente os recibos que realmente foram gerados.
    # A lista é por entrega/recibo, sem agrupamento por fornecedor.
    entregas = c.execute("""
        SELECT
            r.id,
            r.numero_recibo,
...
def detalhe_pedido(recebimento_id):
    ...
    # "responsavel_nfe" e "data_nfe" não existem na tabela (nunca foram
    # adicionadas ao schema); ficam None para manter compatibilidade com
    # o template, que já lida com a ausência desses dados.
...
def _validar_itens_entrega(referencias, entregues):
    ...
    for i, referencia in enumerate(referencias):
        ...
        # Impede que a mesma carenagem seja informada mais de uma vez
        # na mesma entrega, inclusive com diferença apenas de maiúsculas/minúsculas.
